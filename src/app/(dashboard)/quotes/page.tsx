        ...
        <div className="p-8 md:p-10 h-full max-w-7xl mx-auto">
            <header className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button onClick={() => setShowInvoicePreview(true)} className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold transition-all shadow-sm active:scale-95 flex items-center gap-2">
            ...
            {showInvoicePreview ? (
                <div className="glass-panel rounded-[2.5rem] p-8 mt-4 transition-all animate-in fade-in duration-500">
                    <button onClick={() => setShowInvoicePreview(false)} className="mb-6 text-sm font-semibold text-black/50 hover:text-black flex items-center gap-2 transition-colors">
                        &larr; Back to Quotes
                    </button>
                    <div className="w-full overflow-hidden bg-black/5 dark:bg-white/5 rounded-[2rem] p-8 border border-black/5 dark:border-white/5 relative">
                        <InvoicePDF data={DEMO_INVOICE} />
                    </div>
                </div>
            ) : (
                <div className="glass-panel rounded-[2.5rem] p-8 min-h-[400px] flex items-center justify-center flex-col text-center">
                    ...
                    <button onClick={() => setShowInvoicePreview(true)} className="text-sm font-semibold text-blue-500 hover:text-blue-600 transition-colors">Generate Demo Invoice &rarr;</button>
                </div>
            )}
        </div>
    );
